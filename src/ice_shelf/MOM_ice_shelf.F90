...
subroutine shelf_calc_flux(sfc_state_in, fluxes_in, Time, time_step_in, CS)
  ...
  real, dimension(SZDI_(CS%grid),SZDJ_(CS%grid)) :: &
    u_shelf  !< the zonal velocity of the ice shelf/sheet on q-points (B grid) [L T-1 ~> m s-1]
  real, dimension(SZDI_(CS%grid),SZDJ_(CS%grid)) :: &
    v_shelf  !< the meridional velocity of the ice shelf/sheet on q-points (B grid) [L T-1 ~> m s-1]
